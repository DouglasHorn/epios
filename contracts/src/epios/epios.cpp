...
void epios::crcountry(std::string country_name) {
  require_auth(_self);
  country_index country_table(_self, scope_all);
  country_table.emplace(_self, [&](auto &country_to_add) {
    country_to_add.country_id = country_table.available_primary_key();
...
coupons_index::const_iterator epios::apprcoupon(uint64_t coupon_key, uint64_t country_id, eosio::checksum256 secret_key) {
  coupons_index coupons_table(_self, scope_all);
  ...
  eosio::check(secret_key == iter_coupon->secret_key_hash, "Wrong secret key");

  coupons_table.modify(iter_coupon, _self, [&](auto &coupon) {
    coupon.status = uint8_t(unused);
  });
...
void epios::apprlab(eosio::name country_manager_name, std::string name,
  ...
  auto lab_table = lab_index(_self, scope_all);
...
void epios::apprseller(eosio::name country_manager_name, std::string name,
  ...
  auto seller_table = seller_index(_self, scope_all);
...
void epios::delseller(eosio::name country_manager_name, uint16_t id) {
  ...
  seller_index seller_table(_self, scope_all);
...
void epios::check_country_exists(uint64_t country_id) {
  country_index country_table(_self, scope_all);
  eosio::check(country_table.find(country_id) != country_table.end(),
               "No country with such ID");
}

EOSIO_DISPATCH(
    epios,
    (crcountry)(crmanager)(crcntmanager)(crcoupon)(apprlab)(apprseller)(posttestres)(delmanager)(delcountmngr)(delseller))